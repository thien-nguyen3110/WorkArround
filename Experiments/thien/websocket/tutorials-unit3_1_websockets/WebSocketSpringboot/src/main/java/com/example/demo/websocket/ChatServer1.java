...
public class ChatServer1 {
    ...
    public void onOpen(Session session, @PathParam("username") String username) throws IOException {
        ...
        if (usernameSessionMap.containsKey(username)) {
            session.getBasicRemote().sendText("Username already exists . Please choose another username");
            session.close();
        }
        else {
            ...
            sendMessageToPArticularUser(username, "Welcome to the chat server, "+username + "! Try /help for commands");

            // send to everyone in the chat
            broadcast("User: " + username + " has Joined the Chat. Current users: " + getUserList());
        }
    }
    ...
    public void onClose(Session session) throws IOException {
        ...
        broadcast(username + " disconnected.Current user: " + getUserList());
    }
